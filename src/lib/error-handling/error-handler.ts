...
export class ErrorHandler {
    ...
    public async listenToErrorEvents(server: Server): Promise<void> {
        ...
        process.on('SIGTERM', async () => {
            this.log.error(`App received SIGTERM event, attempting to gracefully close the server`);
            await this.termServerAndExit();
        });

        process.on('SIGINT', async () => {
            this.log.error(`App received SIGINT event, attempting to gracefully close the server`);
            await this.termServerAndExit();
        });
    }
